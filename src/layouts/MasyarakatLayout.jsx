...
import { 
  Home, FileText, PlusCircle, LogOut, Bot, User, Bell, Menu, X, Loader2, Info, Clock, MessageSquare, Calendar, Trash2 
} from 'lucide-react';
...
export default function MasyarakatLayout({ children }) {
              ...
              if (change.type === "added") {
                ...
                if (now - notifTime < 5000) {
                  if ("Notification" in window && Notification.permission === "granted") {
                    const systemNotif = new Notification("Portal DP3A: " + newNotif.title, {
  ...
  const menuItems = [
    ...
    { path: '/masyarakat/form', label: 'Buat Pengaduan', icon: FileText },
    { path: '/masyarakat/complaints', label: 'Pengaduan Saya', icon: PlusCircle },
    { path: '/masyarakat/ai-consult', label: 'Konsultasi AI', icon: Bot },
  ];
      ...
      >
        <div className="w-[280px] h-full flex flex-col p-6">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 overflow-hidden">
                ...
                <p className="text-[10px] text-purple-200 uppercase tracking-wider mt-0.5">Masyarakat</p>
              </div>
            </div>
          </div>

          <nav className="flex-1 space-y-2">
            <p className="text-[10px] text-purple-300 mb-4 px-4 font-bold tracking-[0.2em] uppercase opacity-60">Utama</p>
            {menuItems.map((item) => {
              const Icon = item.icon;
              const isActive = location.pathname === item.path || (location.pathname === '/masyarakat/dashboard' && item.path === '/masyarakat');
          ...
          <div className="mt-auto pt-6 border-t border-white/10">
            <button
              className="w-full flex items-center justify-start text-purple-100 hover:bg-white/10 hover:text-white rounded-xl py-4 px-4 transition-all group font-semibold"
        ...
        <header className="h-20 bg-white border-b border-purple-100 px-8 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-4">
            <button 
              ...
              className="p-2 text-[#4B2C82] hover:bg-purple-50 rounded-lg transition-all"
            >
              <Menu className="w-6 h-6" />
            </button>
            <div className="hidden sm:block">
              <h2 className="text-xl font-bold text-[#4B2C82]">
                {menuItems.find(i => i.path === location.pathname)?.label || 'Edit Profil'}
              </h2>
              <p className="text-xs text-gray-400 mt-0.5 font-medium uppercase tracking-wider">Layanan Pengaduan Banjarmasin</p>
            </div>
          </div>
                    ...
                    >
                      <div className="p-4 border-b border-purple-50 flex items-center justify-between bg-gray-50/50">
                        <div className="flex items-center gap-2">
                          <h3 className="font-black text-[#4B2C82]">Notifikasi</h3>
                          {unreadCount > 0 && (
                              ...
                              <div className="flex gap-3">
                                <div className="mt-0.5 shrink-0">
                                  {notif.type === 'system' ? <Info className="w-5 h-5 text-blue-500" /> :
                                   notif.type === 'tanggapan' ? <MessageSquare className="w-5 h-5 text-purple-500" /> :
                                   notif.type === 'jadwal' ? <Calendar className="w-5 h-5 text-yellow-500" /> :
                                   notif.type === 'chat' ? <Bot className="w-5 h-5 text-green-500" /> :
          ...
          <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-gray-500 font-medium mb-8">
                Apakah Anda yakin ingin keluar? Anda harus masuk kembali untuk memantau laporan pengaduan Anda.
              </p>
